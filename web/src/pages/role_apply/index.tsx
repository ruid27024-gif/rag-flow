...
const RoleListPage: React.FC = () => {
  ...
  // ========== 新增：申请角色 ==========
  const handleApplyRole = async (record: RoleItem) => {
    if (!record.enabled) {
      message.warning('该角色已禁用，无法申请');
      return;
    }

    try {
      const res = await fetch('/v1/role/apply', {
        method: 'POST',
        headers: authHeaders,
        credentials: 'include',
        body: JSON.stringify({
      ...
      // 业务成功条件：data 为 true（或非空对象）
      const isSuccess = result.data === true;

      if (isSuccess) {
        message.success('申请已提交，请等待审批');
      } else {
        const msg = result.message || '申请失败';
        // 检查是否为重复申请（根据后端返回的错误信息）
        if (msg.includes('已提交过') || msg.includes('审批中')) {
          message.warning(msg);
        } else {
          message.error(msg);
        }
      ...
      render: (_: any, record: RoleItem) => (
        <Space size={8}>
          {record.enabled && (
            <Button
              size="small"
              onClick={() => handleApplyRole(record)}
              style={{
                borderColor: '#1890ff',
                color: '#1890ff',
                borderRadius: 4,
