...
export const useSelectDerivedConversationList = () => {
  ...
  const addTemporaryConversationWithNewDialog = useCallback(async () => {
    const res = await fetch('/v1/debug/create_dialog_from_config', {
      method: 'POST',
      headers: {
    ...
    if (res.retcode !== 0 || !res.data?.id) return;
  ...
  }, [setConversationBoth, t, prologue, conversationList]);
