...
import { ChevronLeft, FileSearch, Search, Settings } from 'lucide-react';
...
export default function SearchPage() {
  ...
  const [isSearching, setIsSearching] = useState(false);
  const { data: SearchData } = useFetchSearchDetail();
  const { beta } = useFetchTokenListBeforeOtherStep();
  ...
  const [currentMode, setCurrentMode] = useState<SearchMode>('mode3');
   ...
   */
  useEffect(() => {
    const modeFromServer = SearchData?.search_config?.mode as
      | SearchMode
      | undefined;
  ...
  const handleModeChange = useCallback(
    async (mode: SearchMode) => {
      if (!SearchData || savingMode) return;
      ...
      try {
        ...
        console.error('update search mode failed:', error);
      } finally {
        setSavingMode(false);
      }
    },
    [SearchData, savingMode, tenantId, updateSearch],
  );
