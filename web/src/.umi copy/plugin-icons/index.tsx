...
export const Icon = React.forwardRef<HTMLSpanElement, IUmiIconProps>(
  (props, ref) => {
    const {
      icon,
      hover,
    ...
    const HoverComponent = hover
      ? iconsMap[normalizeIconName(alias[hover] || hover)]
      : null;
    ...
    const spanClassName = HoverComponent
      ? 'umiIconDoNotUseThis '
      : '' + className;
    ...
    return (
      <span role="img" ref={ref} {...spanClass} style={style}>
        ...
        {HoverComponent ? (
          <HoverComponent
            {...extraProps}
            className={'umiIconDoNotUseThisHover ' + cls}
            style={svgStyle}
          />
        ) : null}
      </span>
...
function camelCase(str: string) {
  return str
    .replace(/\//g, '-')
    .replace(/-([a-zA-Z]|[1-9])/g, (g) => g[1].toUpperCase());
}
