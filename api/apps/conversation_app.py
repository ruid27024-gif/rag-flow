...
async def completion():
    ...
    if not msg:
        return get_data_error_result(message="No valid message found!")
    ...
    try:
        ...
        # --------------------------------------------------
        # 统计前端请求中已经存在的有效 assistant 数量。
        #
        # 正常提问：
        #   user1, assistant1, user2
        #   existing_assistant_count = 1
        #
        # 重新生成：
        #   前端会删除旧 assistant2，仍然传：
        #   user1, assistant1, user2
        #   existing_assistant_count = 1
        #
        # 所以旧 reference2 会被截断。
        ...
        # 前端重新生成时，旧 assistant 在这里被删除
        conv.message = deepcopy(req["messages"])

        # 对齐已有的 reference
        old_references = [
            reference
            for reference in (conv.reference or [])
            if reference
        ]

        conv.reference = old_references[:existing_assistant_count]

        # 本次生成的新 reference 占位
        conv.reference.append({
            "chunks": [],
            "doc_aggs": [],
            "message_id": message_id,
        })

        reference_index = len(conv.reference) - 1
        ...
        if chat_model_id:
            if not TenantLLMService.get_api_key(
                ...
                return get_data_error_result(
                    message=f"Cannot use specified model {chat_model_id}."
                )
        ...
        async for ans in async_chat(dia, msg, **chat_req):
