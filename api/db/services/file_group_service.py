...
from api.db.db_models import DB, Document, File, File2Document, Knowledgebase, Task, File_Group
...
class FileGroupService(CommonService):
    ...
    def get_by_pf_id(cls, tenant_id, pf_id, page_number, items_per_page, orderby, desc, keywords):
        ...
        if settings.REFERENCE_TENANT_ID:
            conditions.append(cls.model.tenant_id != settings.REFERENCE_TENANT_ID)
    ...
    @classmethod
    @DB.connection_context()
    def get_kb_folder_group(cls, tenant_id):
        # Get dataset folder for tenant
        # Args:
        #     tenant_id: Tenant ID
        # Returns:
